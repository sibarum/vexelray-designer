# Icons, and why the toolbar is words

> **Note from a mistake.** The first toolbar was twelve Unicode marks and every one of them drew as a box. This
> records why, so it is not rediscovered, and what the actual upgrade path is.
...
The prerequisite is nothing — this is buildable today. It was not done in the prototype only because words got
the tool working in one pass, and a drawn icon set is a design job rather than a bug fix.
